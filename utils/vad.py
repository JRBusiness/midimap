"""
Voice Activity Detection (VAD) utilities for piano note and pedal detection.
"""
...
def note_detection_with_onset_offset_regress(
    ...
    for i in range(onset_output.shape[0]):
        if onset_output[i] == 1:
            ...
            if bgn:
                ...
                output_tuples.append([
                    bgn, fin, onset_shift_output[bgn], 
        ...
        if bgn and i > bgn:
            ...
            # End note using whichever happens first: offset or frame fade
            should_end = False
            fin = None
            
            if offset_occur and frame_disappear:
                # Both detected - use earliest
                fin = min(offset_occur, frame_disappear)
                should_end = True
            elif offset_occur:
                fin = offset_occur
                should_end = True
            elif frame_disappear:
                fin = frame_disappear
                should_end = True
            elif i - bgn >= 300:
                # Fallback: max 3 seconds per note
                fin = i
                should_end = True
            elif i == onset_output.shape[0] - 1:
                # End of audio
                fin = i
                should_end = True
                
            if should_end and fin is not None:
                output_tuples.append([
                    bgn, fin, onset_shift_output[bgn], 
